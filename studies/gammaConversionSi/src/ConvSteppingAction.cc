...
ConvSteppingAction::ConvSteppingAction(ConvEventAction* aEventAction,
                                       const ConvDetectorConstruction* aDetector)
  : fEventAction(aEventAction), fDetector(aDetector)
{}
...
void ConvSteppingAction::UserSteppingAction(const G4Step* aStep)
{
  ...
  // Accumulate the path inside the block rather than reading the total track
  // length, so the distance flown through the vacuum upstream is excluded
  const G4VPhysicalVolume* prePhysical = aStep->GetPreStepPoint()->GetPhysicalVolume();
  if (prePhysical != nullptr
      && prePhysical->GetLogicalVolume() == fDetector->GetBlockVolume())
  {
    fEventAction->AddPathInBlock(aStep->GetStepLength());
  }

  const G4VProcess* process = aStep->GetPostStepPoint()->GetProcessDefinedStep();
  if (process == nullptr || process->GetProcessName() != "conv") return;

  fEventAction->SetConversion(aStep->GetPostStepPoint()->GetPosition());
