...
G4VPhysicalVolume* ConvDetectorConstruction::Construct()
{
  ...
  G4cout << "ConvDetectorConstruction: " << fMaterialName << " block "
         ...
         << G4BestUnit(fBlockThickness, "Length") << " (Z = " << GetBlockZ() << ")" << G4endl;

  return worldPhysical;
}

G4double ConvDetectorConstruction::GetBlockZ() const
{
  if (fBlockVolume == nullptr) return 0.;
  // Single-element NIST material: the atomic number of its one element.
  return fBlockVolume->GetMaterial()->GetElement(0)->GetZ();
}
